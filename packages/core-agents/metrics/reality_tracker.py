...
class RealityMetrics:
    ...
    def __init__(self):
        ...
        logger.info("🎯 Reality Metrics Tracker (Persistent) synchronized")
    ...
    def add_intelligence(self, category: str, ticker: str, data: Any):
        ...
        logger.debug(f"🧠 Intelligence added: {category} for {ticker}")
    ...
    async def check_pending_outcomes(self, market_agent: MarketDataAgent):
        ...
        async with async_session() as session:
            ...
            logger.info(f"🔍 Checking outcomes for {len(pending)} pending decisions")
            ...
            for record in pending:
                try:
                    ...
                    logger.error(f"❌ Failed to check outcome for {record.id}: {e}")
    ...
    async def update_outcome(
        ...
        async with async_session() as session:
            ...
            if not record:
                logger.warning(f"⚠️ Decision {decision_id} not found in DB")
                return
            ...
            await session.commit()
            logger.info(f"✅ Updated outcome for {decision_id}: {record.outcome_status} ({record.outcome_roi:.2f}%)")
